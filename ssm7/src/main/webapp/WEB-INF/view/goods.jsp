...
<body class="gray-bg">
	...
	<!-- 附表新增和修改对话框 -->
	<div class="modal fade" id="modal_user_types" role="dialog" aria-labelledby="modal_user_types" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_types" method="post" action="reserveTypes.htm">
						<input type="hidden" name="typeid" id="hidden_txt_typeid" value=""/>
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								<td>用户名：</td>
						...
						<div class="modal-footer">
							<button type="button" class="btn btn-primary"  id="submit_form_types">保存</button>
							<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
						</div>
					</form>
	...
	$(function () {
	    ...
	    $("#submit_form_types").click(function(){
	    	$("#form_types").submit();
	    });
	});
	...
	var ButtonInit = function () {
	    ...
	    oInit.Init = function () {
	    	...
	    	$("#btn_types").click(function(){
	    		$('#password').attr("readOnly",false).val(getSelection.password);
	    		$("#form_types").resetForm();
	    		document.getElementById("hidden_txt_typeid").value='';
	    		$('#modal_user_types').modal({backdrop: 'static', keyboard: false});
				$('#modal_user_types').modal('show');
	        });
	    		...
	    		$.ajax({
	    		    ...
	    		    success:function(res){
	    		    	if(res.success){
	    		    		 var xary=[];
	    		    		 var yary=[];
	    		    		res.data.forEach(function (goodsVo) {
								xary.push(goodsVo.tname);
								yary.push({value:goodsVo.num,name:goodsVo.tname});
							})
